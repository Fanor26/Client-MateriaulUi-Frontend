...
const Estudiantespas = (props) => {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:5000/primeroasec/listarEstudiantes")
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:5000/tareas/listarTareasCurso/"+id,{
        ...
        <Grid container spacing={2} className={classes.container}>
          ...
          {estudiantes.map((estudiante, key) => (
            <Grid item xs={12} sm={4} md={4} lg={4} xl={4} key={key}>
              <Link
                to={{
                  pathname: `/estudiante/${estudiante._id}`,
                }}
              >
                <h2>{estudiante.primernombre}</h2>
              </Link>

              <Paper elevation={10}>
            
                <div className="p-2">
                  <h3>{estudiante.primernombre}</h3>
                </div>
                <h2>{estudiante.segundonombre}</h2>
                <p>{estudiante.tercernombre}</p>
                <p>{estudiante.apellidopaterno}</p>
                <p>{estudiante.apellidomaterno}</p>
