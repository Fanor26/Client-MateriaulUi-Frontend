...
const Login =(props)=>{
  ...
  const usuario={correo,contrasena}
   const respuesta = await Axios.post('http://localhost:5000/escuela/login',usuario); 
  ...
  if(mensaje!=='Bienvenido') {
    ...
    const gestion = respuesta.data.gestion
    const idEscuela = respuesta.data.id
  
    sessionStorage.setItem(' token',token )
    sessionStorage.setItem('nombre',nombre) 
    sessionStorage.setItem('gestion',gestion) 
    sessionStorage.setItem('idEscuela',idEscuela) 
